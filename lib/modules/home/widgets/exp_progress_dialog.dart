...
class _ExpProgressDialogState extends State<ExpProgressDialog>
  ...
  void initState() {
    ...
    _progressAnimation = AlwaysStoppedAnimation(_initialProgress);
  ...
  void _animateProgress(int addedExp) {
    ...
    _progressAnimation =
        Tween<double>(begin: _initialProgress, end: _targetProgress).animate(
          CurvedAnimation(
    ...
    _progressController.addListener(() {
      setState(() {
        ...
        _displayExp =
            widget.animal.currentExpMinutes +
            (addedExp * progressRatio).toInt();
      });
    });
  ...
  void _claim(int multiplier) async {
    ...
    int totalAddedExp = widget.baseExp;
    ...
    if (multiplier > 1) {
      ...
      final rewardEarned = await adService.showRewardedAd(
        onRewardEarned: () {
          Get.snackbar(
            adService.adsDisabledForDevice ? "보너스 적용" : "광고 시청 완료!",
            "경험치를 2배로 획득했습니다! 🎉",
      ...
      totalAddedExp = widget.baseExp * multiplier;
    }
    ...
    // 애니메이션이 끝나길 기다림 (1500ms 애니메이션 + 여유시간)
    await Future.delayed(const Duration(milliseconds: 1700));

    if (!mounted) return;

    // 다이얼로그 닫기 (모든 오버레이를 닫아 Snack-bar 충돌 방지)
    Get.back(closeOverlays: true);
    
    // 다이얼로그가 완전히 닫힐 때까지 아주 짧게 대기 후 콜백 실행 (컨텍스트 충돌 방지)
